...
impl Evm {
    ...
    pub fn step(&mut self) -> Result<(), EvmError> {
        ...
        match op {
            ...
            // Arithmetic
            ADD => {
                self.binop(|a, b| a.overflowing_add(b).0);
                self.gas_dec(3)?;
                self.pc += 1;
            }
            MUL => {
                self.binop(|a, b| a.overflowing_mul(b).0);
                self.gas_dec(5)?;
                self.pc += 1;
            }
            SUB => {
                self.binop(|a, b| a.overflowing_sub(b).0);
                self.gas_dec(3)?;
                self.pc += 1;
            }
            DIV => {
                self.binop(|a, b| if b.is_zero() { U256::zero() } else { a / b });
                self.gas_dec(5)?;
                self.pc += 1;
            }
            ...
            // Env opcodes
            ADDRESS => {
                self.push(h160_to_u256(self.address.unwrap_or_default()))?;
                self.gas_dec(2)?;
                self.pc += 1;
            }
            BALANCE => {
            ...
            ORIGIN => {
                self.push(h160_to_u256(self.origin.unwrap_or_default()))?;
                self.gas_dec(2)?;
                self.pc += 1;
            }
            CALLER => {
                self.push(h160_to_u256(self.caller.unwrap_or_default()))?;
                self.gas_dec(2)?;
                self.pc += 1;
            }
            CALLVALUE => {
            ...
            EXTCODECOPY => {
                ...
                let code = self
                    ...
                    .map(|a| a.code.clone())
                    .unwrap_or_default();
                let m = u256_to_usize(mem_offset);
                let c = u256_to_usize(code_offset);
                let s = u256_to_usize(size);
                self.charge_memory(m + s)?;
                self.ensure_memory(m + s);
                for i in 0..s {
                    self.memory[m + i] = *code.get(c + i).unwrap_or(&0);
                }
                self.gas_dec(100 + ((s as i128 + 31) / 32))?;
                self.pc += 1;
            }
            RETURNDATASIZE => {
                self.push(U256::from(self.last_return_data.len()))?;
                self.gas_dec(2)?;
                self.pc += 1;
            }
            RETURNDATACOPY => {
                ...
                let size = self.pop()?;
                let m = u256_to_usize(mem_offset);
                let d = u256_to_usize(data_offset);
                let s = u256_to_usize(size);
                self.charge_memory(m + s)?;
                self.ensure_memory(m + s);
                for i in 0..s {
                    let v = *self.last_return_data.get(d + i).unwrap_or(&0);
                    self.memory[m + i] = v;
                }
            ...
            COINBASE => {
                self.push(h160_to_u256(self.block.coinbase))?;
                self.gas_dec(2)?;
                self.pc += 1;
            }
            TIMESTAMP => {
                self.push(U256::from(self.block.timestamp))?;
                self.gas_dec(2)?;
                self.pc += 1;
            }
            NUMBER => {
                self.push(U256::from(self.block.number))?;
                self.gas_dec(2)?;
                self.pc += 1;
            }
            DIFFICULTY_PRAND => {
            ...
            GASLIMIT_OP => {
                self.push(self.block.gas_limit)?;
                self.gas_dec(2)?;
                self.pc += 1;
            }
            CHAINID => {
                self.push(self.block.chain_id)?;
                self.gas_dec(2)?;
                self.pc += 1;
            }
            SELFBALANCE => {
                let bal = self
                    .address
                    .and_then(|a| self.world.as_ref().and_then(|w| w.accounts.get(&a)))
                    .map(|ac| ac.balance)
                    .unwrap_or_else(U256::zero);
                self.push(bal)?;
                self.gas_dec(5)?;
            ...
            SSTORE => {
                ...
                let cost = if current.is_zero() && !val.is_zero() {
                    20_000
                } else if !current.is_zero() && val.is_zero() {
                    5_000
                } else {
                    2_900
                };
                self.gas_dec(cost)?;
                if !current.is_zero() && val.is_zero() {
                    self.refund += 15_000;
                }
                self.sstore(key, val);
                self.pc += 1;
            ...
            JUMP => {
                ...
                if !self.jumpdests.contains(&d) {
                    return Err(EvmError::InvalidJump(d));
                }
                self.gas_dec(8)?;
                self.pc = d;
            }
            ...
            GAS => {
                let g = if self.gas > 0 { self.gas as u128 } else { 0 };
            ...
            CALLDATALOAD => {
                ...
                let slice = if o < self.calldata.len() {
                    &self.calldata[o..self.calldata.len().min(end)]
                } else {
                    &[]
                };
                let start = 32 - slice.len();
                if !slice.is_empty() {
                    buf[start..start + slice.len()].copy_from_slice(slice);
                }
            ...
            CALLDATACOPY => {
                ...
                self.ensure_memory(m + s);
                for i in 0..s {
                    let v = if d + i < self.calldata.len() {
                        self.calldata[d + i]
                    } else {
                        0
                    };
            ...
            x if (PUSH1..=PUSH32).contains(&x) => {
                ...
                let start = self.pc + 1;
                let end = start + n;
                let slice = if end <= self.code.len() {
                    &self.code[start..end]
                } else {
                    &[]
                };
            ...
            x if (LOG0..=LOG4).contains(&x) => {
                ...
                let o = u256_to_usize(mstart);
                let s = u256_to_usize(msize);
                self.ensure_memory(o + s);
            ...
            CALL => {
                ...
                let io = u256_to_usize(in_off);
                let isz = u256_to_usize(in_sz);
                let oo = u256_to_usize(out_off);
                let osz = u256_to_usize(out_sz);
                self.charge_memory(io + isz)?;
                self.ensure_memory(io + isz);
                self.charge_memory(oo + osz)?;
                self.ensure_memory(oo + osz);
                    ...
                    if let Some(w) = &mut self.world {
                        ...
                        if from_acc.balance >= value {
                            ...
                            if let Some(pc_ret) = precompile(to_h, &input) {
                                ...
                                let mut child = Evm::new(
                                    code,
                                    EvmConfig {
                                        gas_limit: (forward
                                            + if !value.is_zero() { 2300 } else { 0 })
                                            as i128,
                                ...
                                if let Err(_e) = child.run() {
                                    ...
                                    if success {
                                        if let Some(child_world) = child.world.take() {
                                            *w = child_world;
                                        }
                                    }
                                }
                ...
                // write return
                for i in 0..osz {
                    self.memory[oo + i] = *ret.get(i).unwrap_or(&0);
                }
            ...
            CREATE => {
                ...
                let o = u256_to_usize(offset);
                let s = u256_to_usize(size);
                self.charge_memory(o + s)?;
                self.ensure_memory(o + s);
                let init = self.memory[o..o + s].to_vec();
                let (success, created) = if let Some(w) = &mut self.world {
                    ...
                    if acc.balance >= value {
                        ...
                        let created = create_address(from, nonce);
                        acc.balance -= value;
                        let entry = w_clone.accounts.entry(created).or_default();
                        entry.balance += value;
                        let mut child = Evm::new(
                            init,
                            EvmConfig {
                        ...
                        if let Err(_e) = child.run() {
                            ...
                            let success = !matches!(child.halted, Some(Halt::Revert));
                            if success {
                                let code = child.return_data.clone();
                                if let Some(mut child_world) = child.world.take() {
                                    let e = child_world.accounts.entry(created).or_default();
                                    e.code = code;
                                    *w = child_world;
                                }
                            }
                ...
                if success {
                    self.push(h160_to_u256(created))?;
                } else {
                    self.push(U256::zero())?;
                }
                self.gas_dec(32000)?;
            ...
            CREATE2 => {
                ...
                let (success, created) = if let Some(w) = &mut self.world {
                    ...
                    let acc = w_clone.accounts.entry(from).or_default();
                    if acc.balance >= value {
                        let created = create2_address(from, salt, &init);
                        acc.balance -= value;
                        let entry = w_clone.accounts.entry(created).or_default();
                        entry.balance += value;
                        let mut child = Evm::new(
    ...
    fn sload(&self, key: U256) -> U256 {
        if let Some(w) = &self.world {
            if let Some(addr) = self.address {
                if let Some(acc) = w.accounts.get(&addr) {
                    return *acc.storage.get(&key).unwrap_or(&U256::zero());
                }
            }
        }
...
fn call_gas(available: i128, requested: u128, has_value: bool) -> (u128, u64) {
    ...
    let avail_after_base = if available > (base as i128) {
        (available as u128) - base as u128
    } else {
        0
    };
...
fn create2_address(from: H160, salt: U256, init: &[u8]) -> H160 {
    use tiny_keccak::{Hasher, Keccak};
    let mut ih_out = [0u8; 32];
    let mut ih = Keccak::v256();
    ih.update(init);
    ih.finalize(&mut ih_out);
    let mut out = [0u8; 32];
    let mut k = Keccak::v256();
    k.update(&[0xff]);
    k.update(from.as_bytes());
    let mut sb = [0u8; 32];
    salt.to_big_endian(&mut sb);
    k.update(&sb);
    k.update(&ih_out);
    k.finalize(&mut out);
    H160::from_slice(&out[12..])
